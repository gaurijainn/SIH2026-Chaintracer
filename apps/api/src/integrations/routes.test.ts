...
beforeAll(() => {
  ...
  server = createApp(deps, { integrations: fakeServices }, testSecurity()).listen(0);
...
describe('POST /integrations/sahyog/submit', () => {
  it('returns a sandbox-marked submission result', async () => {
    const res = await authFetch()(`${base}/integrations/sahyog/submit`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ notice: { a: 1 } }) });
  ...
  it('400s when notice is missing', async () => {
    const res = await authFetch()(`${base}/integrations/sahyog/submit`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({}) });
...
describe('POST /integrations/ncrp/sync', () => {
  it('returns a sandbox-marked sync result', async () => {
    const res = await authFetch()(`${base}/integrations/ncrp/sync`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ notice: { a: 1 } }) });
    expect(res.status).toBe(201);
    const body = await res.json();
