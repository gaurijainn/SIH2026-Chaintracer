...
beforeAll(async () => {
  ...
  server = createApp(deps, { freezeNotices }).listen(0);
  base = `http://127.0.0.1:${(server.address() as AddressInfo).port}/api/v1`;
}, 30000);

afterAll(async () => {
  await prisma.freezeNotice.deleteMany({ where: { caseId } });
  await prisma.auditLog.deleteMany({ where: { entity: 'FreezeNotice' } });
...
describe('B9 freeze-notice golden path (real Postgres + real mock SAHYOG server)', () => {
  it('drafts, approves, and sends a freeze notice, persisting a real submissionId', async () => {
    const draftRes = await fetch(`${base}/cases/${caseId}/freeze-notices`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ vaspId }) });
    ...
    const earlySend = await fetch(`${base}/freeze-notices/${drafted.id}/send`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
    expect(earlySend.status).toBe(400);

    const submitRes = await fetch(`${base}/freeze-notices/${drafted.id}/submit`, { method: 'POST' });
    expect(submitRes.status).toBe(200);
    expect((await json(submitRes)).freezeNotice.status).toBe('PENDING_APPROVAL');

    const approveRes = await fetch(`${base}/freeze-notices/${drafted.id}/approve`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
    expect(approveRes.status).toBe(200);
    expect((await json(approveRes)).freezeNotice.status).toBe('APPROVED');

    const sendRes = await fetch(`${base}/freeze-notices/${drafted.id}/send`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
